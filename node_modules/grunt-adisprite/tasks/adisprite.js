...
module.exports = function (grunt) {
    ...
    grunt.registerMultiTask('adisprite', 'Create sprites and updates css.', function () {
        ...
        (function () {
            ...
            var spriteSrcDIR = that.data.srcImg,
                spriteDestDIR = that.data.destImg,
                cssSrcDIR = that.data.srcCss,
                cssDestDIR = that.data.destCss,
            ...
            if(listDIR(spriteSrcDIR).length>0){
                spritelist = listDIR(spriteSrcDIR).map(function (itemDIR) {
                    ...
                    var sprite = setUpSpriteConfig(that.data, src, dest)
                    return sprite;
                })
        ...
        var _retinaSpriteSmithWrapper = function (config, callback) {
            ...
            delete config.sprite;
            spritesmith(config, function (err, result) {
                if (err) { grunt.fatal(err); return callback(err); }
                grunt.file.write(sprite, result.image, { encoding: 'binary' });
                gm(sprite).size(function(err, size){
                    if(err){ grunt.fatal(err); return;}
                    if(size.width%2 ==1 || size.height%2 ==1){
//                        grunt.fail.fatal('warning: all slices are not allow odd pix')
                    }
                    grunt.log.writelns('Done! [Created] -->'+sprite)
                    var tmpResult = result.coordinates
                    for (var key in result.coordinates) {
                        var newKey = path.join(process.cwd(), key).toLowerCase();
